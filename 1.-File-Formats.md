...
Before we begin, it's better to understand a little on how they work.

Usually, Icons are part of the firmware and compiled together with it, so we had to make our own system to load them dynamically. Not all icons are supported for now, but the ones that are should cover most use cases. Animations instead were already being loaded from the SD card (`SD/dolphin`) so we only had to tweak the system to allow loading from Asset Packs.
...
With icons there are quite a few differences and issues we had to solve. First of all, they are usually compiled along with the firmware, so loading them dynamically required a special system. Second, the `.bm` format does not include image width and height, with animations that is stored in meta.txt, so for icons we had to make a special format: `.bmx`, which is `[ 4 bytes width ] + [ 4 bytes height ] + [ standard .bm pixel data ]`, but this is handled by the packer (see below). Other than that we kept the same icon naming scheme and structure, so this should look familiar otherwise.
...
- Since our passport only changes portrait based on mood and not level, the icons are named as `Passport/passport_bad_46x49` instead of `Passport/passport_bad1_46x49` (note the `1` for the level is gone).
...
All the .bm and .bmx struggles are dealt with by the packer system, which is in `scripts/asset_packer.py`; when making your Asset Pack you will only be working with .png images and meta.txt/manifest.txt files. As explained above, packs are made of 2 parts, Anims and Icons, but you don't have to include both - if you only include Anims, then the default SFW/NSFW Icons will be used, and viceversa. You have 2 options: make standalone Asset Packs (recommended), or build them along with the firmware.

#### Standalone Asset Packs

- Make a folder anywhere on your system where you'll put all your source asset packs. If you're not sure, the Desktop is always a good place.
- Here make a folder with the name of your Asset Pack (less than 32 characters). Inside that, make the Anims and/or Icons folders.
- Fill those folders with .png images and meta.txt/manifest.txt files where appropriate, referring to the information and structure above (.bm and .bmx should be .png, and the structure in SD/dolphin_custom is the same as your source packs folder).

  Here is an example of what it should look like:

  ![Source Pack Structure](https://user-images.githubusercontent.com/49810075/215292982-782a32d4-03c9-49c0-b0ce-e896a0110034.png)

- Copy the `scripts/asset_packer.py` file into your source packs folder, right next to your asset packs.
- Run the asset_packer.py script. You'll need Python installed. You might be able to double click it, if not run in console with `python asset_packer.py`.
